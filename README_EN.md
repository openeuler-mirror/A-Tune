...
**Note:** After running `systemctl start atuned`, an error message may be displayed because the authentication certificate is not configured. There are two ways to solve the problem:
...
2. Cancel certificate authentication and use HTTP insecure connections.
...
No matter which method is used, restart services after the setting is complete. For details, see "II. Quick Guide - 2. Manage the A-Tune service - Load and start the atuned and atune-engine services".
...
#### 2.2 (Optional) Install dependencies for the database.
...
To use the database, you should also select either of the following methods to install dependencies for the database application.
...
#### Modify the network and drive figuration in the atuned.cnf file.

Note: If the atuned service is installed by `make install`, NICs and drives have been automatically updated to the default device in the current machine. If you need to collect data from other devices, configure atuned service according to following steps.
...
You can run the following command to query the drive that needs to be specified for data collection or optimization and change the value of the drive configuration item in the **/etc/atuned/atuned.cnf** file to the specified drive.
...
Note: Some data collected by the analysis command are from the drive and NIC configured in the atuned service configuration file (**/etc/atuned/atuned.cnf**). Before executing the command, check whether the configuration items are as expected. To collect data from other NICs or drives, you need to update the atuned service configuration file and restart the atuned service.
...
[A-Tune-UI](https://gitcode.com/openeuler/A-Tune-UI) is a web project based on A-Tune. Please check A-Tune-UI [README](https://gitcode.com/openeuler/A-Tune-UI/blob/master/README.en.md) for details.

IV. How to Contribute

We welcome new contributors to participate in the project, and we are happy to provide guidance for new contributors. Please sign the [CLA](https://clasign.osinfra.cn/sign/gitcode_openeuler-1611298811283968340) before contribution.
