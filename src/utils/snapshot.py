...
save_snapshot = config["feature"][0]["save_snapshot"]
use_snapshot = config["feature"][0]["use_snapshot"]
...
if not snapshot_path_inited:
    ...
    logging.info(f"save_snapshot: {save_snapshot}; use_snapshot: {use_snapshot}")
    if save_snapshot or use_snapshot:
        logging.info(f"snapshot_path: {snapshot_path}")

# 保存快照
def save_snapshot(var, filename):
