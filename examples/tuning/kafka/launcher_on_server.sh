...
if [ -f "./client_ssh_key" ]; then
    ssh -i client_ssh_key root@"$KAFKA_CLIENT_IP" -t "/usr/bin/bash /root/benchmark_on_client.sh"
    scp -i client_ssh_key root@"$KAFKA_CLIENT_IP":/root/kafka_benchmark.log ./
else
