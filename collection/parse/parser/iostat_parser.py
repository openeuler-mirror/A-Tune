...
class IostatParser(base.Parser):
    ...
    def _get_supported_metrics(self):
        """Read the first batch output of iostat and get all supported metrics.
    ...
    def _get_extra_supported_metrics(self):
        ...
        if "aqu-sz" in supported_metrics:
            self._support_blk_sat = True
            self._aqu_sz_name = "aqu-sz"
            return {"blk_sat": ["aqu-sz"]}
        if "avgqu-sz" in supported_metrics:
            self._support_blk_sat = True
            self._aqu_sz_name = "avgqu-sz"
            return {"blk_sat": ["avgqu-sz"]}
        return {}
    ...
    def _check_dev(self):
        ...
        if diff_set:
            raise ValueError("Can not find block device `{}` in file `{}`"
                             .format(','.join(diff_set), self._raw_data_file))
        ...
        with open(self._raw_data_file, 'r') as raw_data_fd:
            ...
            for row_num, line in enumerate(raw_data_fd, 2):
                ...
                if header:
                    ...
                    if self._support_blk_sat:
                        data[dev]["blk_sat"] = data[dev][self._aqu_sz_name]
